### In this script, we first calculate hybridization rates 
### for Sphyrapicus ruber and S. nuchalis by county absed 
### on eBird data. To demonstrate the prevalence of cryptic 
### hybrids, we then compare these estimates to hybrid 
### proportions based on whole genome data generated by Shawn
### Billerman. At the end of the script, we begin to explore 
### the influence of spatial variation in eBird effort on 
### hybridization rate estimates.  
...
ebd_filters <- auk_ebd(f_in_ebd) %>% 
  ...
  auk_date(date = c("*-05-09", "*-06-05")) #the date range over which Shawn collected in 2012
...
county_proportions <- sapply(sapsucker_counties, function(x){
  county_dat <- sapsuckers %>%
    filter(county_code == x)
  hybrid_dat <- county_dat %>% 
    filter(category=="hybrid")
  all_count <- sum(county_dat$observation_count)
  hybrid_count <- sum(hybrid_dat$observation_count)
  hybrid_proportion <- (sum(hybrid_dat$observation_count))/(sum(county_dat$observation_count))
  c(all_count, hybrid_count, hybrid_proportion)
})

# converting to a dataframe
typeof(county_proportions)
county_proportions_df <- as.data.frame(t(county_proportions))

#renaming the columns of the dataframe
names(county_proportions_df)[1:3] <- c("all_count", "hybrid_count", "hybrid_proportion")
...
county_proportions_shawn <- sapply(sapsucker_shawn_counties, function(x){
  county_dat <- sapsucker_shawn %>%
    filter(county == x)
  hybrid_dat <- county_dat %>% 
    filter(category=="hybrid")
  all_count <- nrow(county_dat)
  hybrid_count <- nrow(hybrid_dat)
  hybrid_proportion <- nrow(hybrid_dat)/nrow(county_dat)
  c(all_count, hybrid_count, hybrid_proportion)
})

# converting to a dataframe
typeof(county_proportions_shawn)
county_proportions_shawn_df <- as.data.frame(t(county_proportions_shawn))

#renaming the columns of the dataframe
names(county_proportions_shawn_df)[1:3] <- c("all_count", "hybrid_count", "hybrid_proportion")
